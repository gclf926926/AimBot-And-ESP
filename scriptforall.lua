...
-- BOTÃO 2: Highlight
local ToggleHighlight = criarBotao("Teste 2: OFF", Color3.fromRGB(255, 0, 0), 160)
local highlightAtivado = false
local highlights = {}

-- Função: Adicionar Highlights vermelhos
local function ativarHighlights()
	for _, player in pairs(players:GetPlayers()) do
		if player ~= localPlayer and player.Character then
			local highlight = Instance.new("Highlight")
			highlight.Name = "ManinhoHighlight"
...
-- Evento Botão 2 (Highlight)
ToggleHighlight.MouseButton1Click:Connect(function()
	highlightAtivado = not highlightAtivado
	if highlightAtivado then
		ToggleHighlight.Text = "Teste 2: ON"
		ToggleHighlight.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
		ativarHighlights()
	else
		ToggleHighlight.Text = "Teste 2: OFF"
		ToggleHighlight.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
		desativarHighlights()
	end
end)
